...
func isNonWordChar(re *syntax.Regexp, before bool) (bool, string, error) {
	...
	case syntax.OpLiteral: // a
		logging.Debug().Msgf("[isNonWordChar#literal] op = %v, re = %v, sub = %v", re.Op, re, re.Sub)
		var r rune
	...
	case syntax.OpCharClass: // [a-z0-9]
		logging.Debug().Msgf("[isNonWordChar#charclass] op = %v, re = %v, sub = %v", re.Op, re, re.Sub)
		...
		for i := 0; i < len(re.Rune); i += 2 {
			...
			if from == to {
				...
				case 'ſ', 'K':
					...
					// I don't know why this optimization exists, only that we must pretend they match `\w`.
					// https://github.com/golang/go/blob/a11643df8ff8a575abe4abc7f25d09631424ea49/src/regexp/syntax/parse_test.go#L235-L236
					return false, string(from), nil
				default:
					return !isWordCharacter(from), string(from), nil
				}
			}
			...
			if !ok {
				return true, string(from) + "-" + string(to), nil
			}
		}
