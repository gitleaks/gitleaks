...
#
#     Options:
#       <gitleaks-path>       - path to gitleaks binary to profile
#       <benchmark-repo-path> - git repo to run profile against
#
# SEE ALSO
#     Dave Cheney’s GopherCon 2019 talk on Go profiling:
...
log '========================================================================'
log 'Generating Gitleaks profile data'
log '------------------------------------------------------------------------'

# Warm up filesystem and collect benchmark/profile data
for scan_mode in dir git; do
  ...
  # Generate diagnostics: cpu, mem, trace
  echo "  profile:"
  for profile_mode in cpu mem trace; do
    "${gitleaks_path}" ${scan_mode} "${test_repo_path}" \
      --diagnostics="${profile_mode}" \
      --diagnostics-dir="${profile_dir}" &> /dev/null

    profile_file="$(find "${profile_dir}" -type f -name "${profile_mode}*" -print -quit)"
    ...
    if [[ "${profile_mode}" = "trace" ]]; then
      ...
      echo "      view: go tool pprof -http=localhost:8080 \"${gitleaks_path}\" \"${profile_file}\""
    fi
  done
done

log '------------------------------------------------------------------------'
log "Results saved in: ${base_profile_dir}"
log '========================================================================'
