	...
	for {
		...
		for _, rule := range d.Config.Rules {
			if len(rule.Keywords) == 0 {
				...
				findings = append(findings, d.detectRule(rule, nil, fragment, currentRaw, encodedSegments)...)
				continue
			}
			...
			if len(matchedKeywords) > 0 {
				findings = append(findings, d.detectRule(rule, matchedKeywords, fragment, currentRaw, encodedSegments)...)
			}
		}
...
func (d *Detector) detectRule(r config.Rule, keywords []string, fragment Fragment, currentRaw string, encodedSegments []*codec.EncodedSegment) []report.Finding {
	var (
		findings []report.Finding
		logger   = func() zerolog.Logger {
