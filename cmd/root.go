...
func init() {
	...
	// Feature flags
	rootCmd.PersistentFlags().BoolP("experimental-regexpcheck", "", false, "enables experimental regex validations, identifying pattern issues")
...
func Config(cmd *cobra.Command) config.Config {
	...
	if mustGetBoolFlag(cmd, "experimental-regexpcheck") {
		logging.Warn().Msgf("using experimental regex validations, updates may contain breaking changes!")
		flags.EnableExperimentalPatternChecks.Store(true)
	}
