...
func TestScan(t *testing.T) {
	...
	tests := []struct {
		...
		{
			description: "test fails in bare repo if gitleaks config isn't found",
			opts: options.Options{
				RepoPath:        "../test_data/test_repos/test_repo_bare_no_gitleaks_config",
				ReportFormat:    "json",
				Config:          "../test_data/test_configs/aws_key_file_regex.toml",
				RepoConfig:      true,
				MergeRepoConfig: true,
			},
			wantErr: fmt.Errorf("open ../test_data/test_repos/test_repo_bare_no_gitleaks_config/gitleaks.toml: no such file or directory"),
		},
		{
			description: "test malfromed repo uses blank config, and fails due to not having a worktree",
			opts: options.Options{
				RepoPath:        "../test_data/test_repos/test_repo_malformed",
				ReportFormat:    "json",
				Config:          "../test_data/test_configs/aws_key_file_regex.toml",
				RepoConfig:      true,
				MergeRepoConfig: true,
			},
			wantErr: fmt.Errorf("worktree not available in a bare repository"),
		},
		{
			description: "test ability to use additional config",
			opts: options.Options{
				RepoPath:         "../test_data/test_repos/test_repo_10",
				ReportFormat:     "json",
				AdditionalConfig: "../test_data/test_configs/additional_config_allow.toml",
			},
			wantEmpty: true,
		},
		{
			description: "test ability to use additional config to trigger leak",
			opts: options.Options{
				RepoPath:         "../test_data/test_repos/test_repo_10",
				ReportFormat:     "json",
				Report:           "../test_data/test_additional_config.json",
				Config:           "../test_data/test_configs/blank_config.toml",
				AdditionalConfig: "../test_data/test_configs/aws_key.toml",
			},
			wantPath: "../test_data/test_additional_config.json",
		},
		{
			description: "test get error when supplied additional config is not found",
			opts: options.Options{
				RepoPath:         "../test_data/test_repos/test_repo_10",
				ReportFormat:     "json",
				Report:           "../test_data/test_additional_config_bad_config.json",
				Config:           "../test_data/test_configs/aws_key_file_regex.toml",
				AdditionalConfig: "../test_data/test_configs/xyz.toml",
			},
			wantErr: fmt.Errorf("open ../test_data/test_configs/xyz.toml: no such file or directory"),
		},
	}
